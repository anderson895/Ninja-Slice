...
public class Fruit20 : MonoBehaviour
{
    ...
    private void Slice(Vector3 direction, Vector3 position, float force)
    {
        ...
        if (fruitValue == Level20.Instance.GetCurrentAnswer())
        {
            Debug.Log("Correct fruit sliced. Updating score and displaying the next question...");
            Level20.Instance.AddScore(1);
            Level20.Instance.DisplayNextQuestion();
        }
        else
        {
            Debug.Log($"Wrong fruit sliced. Fruit value: {fruitValue}, Expected: {Level20.Instance.GetCurrentAnswer()}");
            Level20.Instance.LoseHeart();
        }
