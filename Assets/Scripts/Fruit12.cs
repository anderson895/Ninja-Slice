...
public class Fruit12 : MonoBehaviour
{
    ...
    private void Slice(Vector3 direction, Vector3 position, float force)
    {
        ...
        if (fruitValue == Level12.Instance.GetCurrentAnswer())
        {
            Debug.Log("Correct fruit sliced. Updating score and displaying the next question...");
            Level12.Instance.AddScore(1);
            Level12.Instance.DisplayNextQuestion();
        }
        else
        {
            Debug.Log($"Wrong fruit sliced. Fruit value: {fruitValue}, Expected: {Level12.Instance.GetCurrentAnswer()}");
            Level12.Instance.LoseHeart();
        }
