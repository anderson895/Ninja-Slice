...
public class Fruit17 : MonoBehaviour
{
    ...
    private void Slice(Vector3 direction, Vector3 position, float force)
    {
        ...
        if (fruitValue == Level17.Instance.GetCurrentAnswer())
        {
            Debug.Log("Correct fruit sliced. Updating score and displaying the next question...");
            Level17.Instance.AddScore(1);
            Level17.Instance.DisplayNextQuestion();
        }
        else
        {
            Debug.Log($"Wrong fruit sliced. Fruit value: {fruitValue}, Expected: {Level17.Instance.GetCurrentAnswer()}");
            Level17.Instance.LoseHeart();
        }
