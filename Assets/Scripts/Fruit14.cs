...
public class Fruit14 : MonoBehaviour
{
    ...
    private void Slice(Vector3 direction, Vector3 position, float force)
    {
        ...
        if (fruitValue == Level14.Instance.GetCurrentAnswer())
        {
            Debug.Log("Correct fruit sliced. Updating score and displaying the next question...");
            Level14.Instance.AddScore(1);
            Level14.Instance.DisplayNextQuestion();
        }
        else
        {
            Debug.Log($"Wrong fruit sliced. Fruit value: {fruitValue}, Expected: {Level14.Instance.GetCurrentAnswer()}");
            Level14.Instance.LoseHeart();
        }
