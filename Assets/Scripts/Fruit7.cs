...
public class Fruit7 : MonoBehaviour
{
    ...
    private void Slice(Vector3 direction, Vector3 position, float force)
    {
        ...
        if (fruitValue == Level7.Instance.GetCurrentAnswer())
        {
            Debug.Log("Correct fruit sliced. Updating score and displaying the next question...");
            Level7.Instance.AddScore(1);
            Level7.Instance.DisplayNextQuestion();
        }
        else
        {
            Debug.Log($"Wrong fruit sliced. Fruit value: {fruitValue}, Expected: {Level7.Instance.GetCurrentAnswer()}");
            Level7.Instance.LoseHeart();
        }
